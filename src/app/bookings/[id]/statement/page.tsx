 ...
 return <main className={styles.page}>
  ...
  <article className={styles.statement}>
   ...
   <section className={styles.section}><h2>{customerView?'Payment record':'Payout record'}</h2><div className={styles.payment}>
    {customerView?<><div><span>Total paid</span><strong>{money(totalPaid)}</strong></div>{refunded>0&&<><div><span>Refunded</span><strong>{money(refunded)}</strong></div><div><span>Net paid</span><strong>{money(Math.max(0,totalPaid-refunded))}</strong></div></>}<div><span>Payment status</span><strong>{booking.payment?label(booking.payment.status):'Not recorded'}</strong></div></>:<>{cancellationDeduction>0&&<><div><span>Proceeds before fine</span><strong>{money(proceeds+cancellationDeduction)}</strong></div><div><span>Fine deduction</span><strong>−{money(cancellationDeduction)}</strong></div></>}<div><span>{cancellationDeduction>0?'Net transporter payout':'Transporter proceeds'}</span><strong>{money(proceeds)}</strong></div><div><span>Payout status</span><strong>{booking.payment?label(booking.payment.payoutStatus):'Not recorded'}</strong></div>{payoutReleasedAt&&<div><span>Paid on</span><strong>{dateTime(payoutReleasedAt)}</strong></div>}</>}
   </div></section>
