...
export default function Verification() {
  ...
  return <main className="shell dashboardShell verificationShell">
    ...
    <section className="dashboardCard verificationSection"><div className="verificationHeading"><div className="panelIcon">2</div><div><h2>Verification documents</h2><p>Insurance and a driving licence are required. Upload PDF, JPG/JPEG or PNG files, maximum 4 MB each.</p></div></div><form className="documentUploadForm" onSubmit={addDocument}><div className="grid"><div className="field"><label>DOCUMENT TYPE</label><select name="type"><option value="INSURANCE">Insurance certificate</option><option value="DRIVING_LICENCE">Driving licence</option><option value="COMPANY_REGISTRATION">Company registration</option><option value="IDENTITY">Identity</option><option value="OPERATOR_LICENCE">Operator licence</option><option value="OTHER">Other</option></select></div><div className="field fileField"><label>UPLOAD DOCUMENT</label><input name="file" type="file" accept="application/pdf,image/jpeg,image/png,.pdf,.jpg,.jpeg,.png" required /></div><div className="field"><label>INSURER</label><input name="insurer" /></div><div className="field"><label>POLICY / REFERENCE</label><input name="policyNumber" /></div><div className="field"><label>EXPIRY DATE</label><input name="expiresAt" type="date" /></div></div><button className="btn orange" disabled={uploading}>{uploading ? 'Uploading securely…' : 'Upload secure document'}</button></form>
