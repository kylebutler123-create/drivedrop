 ...
 return <main className="shell dashboardShell">
  ...
  {bookings.map(b=>{
   ...
   return <article className="dashboardCard bookingCard" key={b.id}>
    <div className="bookingTop"><div><span className="statusPill successPill">Delivered</span><h2>{b.job.vehicleMake} {b.job.vehicleModel}</h2><p className="bookingPartner">Customer · <b>{b.customer.name}</b></p></div>{b.payment&&<div className="paymentMini"><span>Your proceeds</span><strong>£{(b.payment.transporterProceedsPence/100).toFixed(2)}</strong>{b.payment.cancellationDeductionPence>0&&<small>£{(b.payment.cancellationDeductionPence/100).toFixed(2)} cancellation fine deducted</small>}<small>Payout · {label(b.payment.payoutStatus)}</small></div>}</div>
