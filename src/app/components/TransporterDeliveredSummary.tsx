...
export default function TransporterDeliveredSummary(){
 ...
 return <>
  {createPortal(<><strong>{bookings.length}</strong><span>Completed</span></>,target)}
  {listTarget&&show&&createPortal(<section className="dashboardCard" style={{margin:'18px auto',maxWidth:1180}}><div className="dashboardSectionHeading" style={{marginTop:0}}><div><span className="dashboardEyebrow dark">Delivery history</span><h2>Completed</h2></div><span>{bookings.length} total</span></div>{bookings.length===0?<p className="muted">No completed jobs yet.</p>:bookings.map(b=><article className="dashboardCard" key={b.id}><div className="bookingTop"><div><span className="statusPill successPill">Completed</span><h2>{b.job.vehicleMake} {b.job.vehicleModel}</h2><p className="bookingPartner">Customer · <b>{b.customer.name}</b></p></div>{b.payment&&<div className="paymentMini"><span>Your proceeds</span><strong>{money(b.payment.transporterProceedsPence||0)}</strong><small>{String(b.payment.payoutStatus||'').replaceAll('_',' ')}</small></div>}</div><div className="routeVisual compactRoute"><div><i>●</i><span><small>Collection</small><b>{b.job.collection}</b></span></div><div className="routeLine"/><div><i>●</i><span><small>Delivery</small><b>{b.job.delivery}</b></span></div></div>{b.trackingEvents?.[0]?.createdAt&&<p className="muted">Completed {new Date(b.trackingEvents[0].createdAt).toLocaleDateString('en-GB')}</p>}</article>)}</section>,listTarget)}
 </>;
}
