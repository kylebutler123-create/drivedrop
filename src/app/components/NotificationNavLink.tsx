...
export default function NotificationNavLink(){
 ...
 return <a className={'headerStatusLink notificationsHeaderLink'+(count?' hasUnread':'')} href="/notifications" aria-label={count?'Notifications, '+count+' unread':'Notifications'}><svg className="mobileNavIcon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9"/><path d="M10 21h4"/></svg><span className="notificationDesktopLabel">Notifications</span><span className="notificationMobileLabel">Alerts</span>{count>0&&<span className="navCountBadge">{count>99?'99+':count}</span>}</a>;
}
