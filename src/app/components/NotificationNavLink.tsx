...
export default function NotificationNavLink(){
 ...
 const load=useCallback(async()=>{const r=await fetch('/api/notifications',{cache:'no-store'});if(r.ok){const d=await r.json();setCount(d.unread||0)}},[]);
 useEffect(()=>{load();const t=setInterval(load,5000);const update=()=>load();window.addEventListener('drivedrop:notifications-read',update);return()=>{clearInterval(t);window.removeEventListener('drivedrop:notifications-read',update)}},[load]);
