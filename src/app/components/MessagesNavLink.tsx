...
export default function MessagesNavLink(){
 ...
 useEffect(()=>{refresh();const t=setInterval(refresh,5000);const onRead=()=>refresh();window.addEventListener('drivedrop:messages-read',onRead);return()=>{clearInterval(t);window.removeEventListener('drivedrop:messages-read',onRead)}},[refresh]);
 return <Link href="/messages" style={{display:'inline-flex',alignItems:'center',gap:6}}>Messages{count>0&&<span aria-label={`${count} unread message${count===1?'':'s'}`} style={{display:'inline-flex',alignItems:'center',justifyContent:'center',minWidth:20,height:20,padding:'0 6px',borderRadius:999,background:'#ff7a18',color:'#fff',fontSize:12,fontWeight:800,lineHeight:1}}>{count>99?'99+':count}</span>}</Link>;
}
