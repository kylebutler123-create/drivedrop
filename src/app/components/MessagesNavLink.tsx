...
export default function MessagesNavLink(){
 ...
 useLayoutEffect(()=>{try{const saved=sessionStorage.getItem('drivedrop:unread-messages');if(saved!==null){const value=Number(saved);if(Number.isFinite(value)&&value>=0)setCount(value)}}catch{}},[]);
 const refresh=useCallback(async()=>{try{const r=await fetch('/api/messages/unread-count',{cache:'no-store'});if(r.ok){const d=await r.json();const value=Number(d.count)||0;setCount(value);try{sessionStorage.setItem('drivedrop:unread-messages',String(value))}catch{}}}catch{}},[]);
