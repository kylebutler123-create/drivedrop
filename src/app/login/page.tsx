...
export default function Login(){
  ...
  return <main className="authShell"><section className="authVisual"><div className="authVisualOverlay"/><div className="authVisualContent"><span className="eyebrow">UK VEHICLE TRANSPORT</span><h2>{isTransporter?'Keep your transport business moving.':'Move your vehicle with confidence.'}</h2><p>{isTransporter?'Quote for suitable jobs, manage live deliveries and keep customers updated from one workspace.':'Compare transporter quotes, manage your booking and follow every stage of the delivery.'}</p><div className="authTrust"><span>✓ Verified transporters</span><span>✓ Secure delivery evidence</span><span>✓ Clear booking progress</span></div></div></section><section className="authFormSide"><div className="authCard"><Link className="authBack" href="/">← Back to DriveDrop</Link><span className="dashboardEyebrow dark">Secure account access</span><h1>{heading}</h1><p className="authIntro">{intro}</p><div className="accountSwitch"><Link className={!isTransporter?'active':''} href="/login?account=customer">Customer</Link><Link className={isTransporter?'active':''} href="/login?account=transporter">Transporter</Link></div><form onSubmit={submit}><div className="field"><label>EMAIL</label><input type="email" name="email" autoComplete="email" placeholder="you@example.com" required/></div><div className="field"><label>PASSWORD</label><div className="password-wrap"><input type={showPassword?'text':'password'} name="password" autoComplete="current-password" placeholder="Your password" required/><button type="button" className="password-toggle" onClick={()=>setShowPassword(v=>!v)} aria-label={showPassword?'Hide password':'Show password'}>{showPassword?'Hide':'Show'}</button></div></div>{err&&<div className="formNotice errorNotice">{err}</div>}<button className="btn orange authSubmit">Sign in securely</button></form><div className="authDivider"><span>New to DriveDrop?</span></div><Link className="authSecondary" href={isTransporter?'/register?account=transporter':'/register?account=customer'}>{isTransporter?'Create transporter account':'Create customer account'} →</Link><p className="authFinePrint">Your account credentials are handled securely by DriveDrop.</p></div></section></main>;
}
