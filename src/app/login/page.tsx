  ...
  return <main className={`authPage loginPage ${isTransporter?'transporterLogin':'customerLogin'}`}>
    <section className="authVisual">
      ...
      <div className="authVisualContent">
        ...
        <p>{isTransporter?'Quote for suitable jobs, manage live deliveries and keep customers updated from one workspace.':'Compare transporter quotes, manage your booking and follow every stage of the delivery.'}</p>
        <div className="authTrust">
          <span>✓ Verified transporters</span>
          <span>✓ Secure delivery evidence</span>
          <span>✓ Clear booking progress</span>
        </div>
      </div>
    </section>
    <section className="authPanel">
      <div className="authCard">
        <Link className="backLink" href="/">← Back to DriveDrop</Link>
        <div className="authHeading">
          <span className="dashboardEyebrow dark">Secure account access</span>
          <h2>{heading}</h2>
          <p>{intro}</p>
        </div>
        ...
        <form onSubmit={submit}>
          <div className="field">
            <label>EMAIL</label>
            <input type="email" name="email" autoComplete="email" placeholder="you@example.com" defaultValue={changedEmail} required/>
          </div>
          <div className="field">
            <div className="passwordLabelRow">
              <label>PASSWORD</label>
              <Link className="forgotLink" href={`/forgot-password${account?`?account=${account}`:''}`}>Forgot password?</Link>
            </div>
            <div className="password-wrap">
              <input type={showPassword?'text':'password'} name="password" autoComplete="current-password" placeholder="Your password" required/>
          ...
          {err&&<div className="formNotice errorNotice">{err}</div>}
