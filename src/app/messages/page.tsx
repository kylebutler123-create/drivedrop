...
const supportConversationId=(value:string)=>value.startsWith('support:')?value.slice('support:'.length):null;
...
export default function MessagesPage(){
 ...
 async function markRead(bookingId:string){const supportId=supportConversationId(bookingId);const r=await fetch(supportId?'/api/support/messages':'/api/bookings/messages',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify(supportId?{conversationId:supportId}:{bookingId})});if(r.ok){setUnreadByBooking(x=>({...x,[bookingId]:0}));window.dispatchEvent(new Event('drivedrop:messages-read'))}}
 ...
 async function loadMessages(bookingId:string){
  ...
  try{
   const supportId=supportConversationId(bookingId);const r=await fetch(supportId?`/api/support/messages?conversationId=${encodeURIComponent(supportId)}`:`/api/bookings/messages?bookingId=${encodeURIComponent(bookingId)}`,{cache:'no-store',signal:controller.signal});
 ...
 const adminRecipientLabel=useMemo(()=>{if(me?.role!=='ADMIN'||!selected||selected.status==='SUPPORT'||adminRecipient.current?.bookingId!==selected.id)return null;const id=adminRecipient.current.recipientId;if(id===selected.customer.id)return`Customer · ${selected.customer.name}`;if(id===selected.transporter.id)return`Transporter · ${selected.transporter.name}`;return null},[me,selected]);
 ...
 const visibleBookings=useMemo(()=>sortedBookings.filter(b=>{
  ...
  const other=b.status==='SUPPORT'?(me?.role==='ADMIN'?b.customer.name:'DriveDrop Support'):me?.role==='CUSTOMER'?b.transporter.name:b.customer.name;
 ...
 async function send(e:FormEvent){
  ...
  setNotice('');
  try{
   const supportId=supportConversationId(bookingId);
   let r:Response;
   if(supportId){
    r=await fetch('/api/support/messages',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({conversationId:supportId,body})});
   }else{
    ...
    r=await fetch('/api/bookings/messages',{method:'POST',body:f});
   }
   const d=await r.json().catch(()=>null);
 ...
 return <main className="shell dashboardShell messagesShell"><Link className="backLink" href={dashboard}>← Back to dashboard</Link><header className="dashboardHero messagesHero"><div><span className="dashboardEyebrow">Marketplace conversations</span><h1>Your messages</h1><p>Keep delivery conversations and DriveDrop Support messages together in one place.</p></div><div className="messagesHeroStats"><button type="button" onClick={()=>setInboxView('ALL')} aria-pressed={inboxView==='ALL'} className={inboxView==='ALL'?'isActive':''}><span>Conversations</span><strong>{bookings.length}</strong></button><button type="button" onClick={()=>setInboxView('UNREAD')} aria-pressed={inboxView==='UNREAD'} className={inboxView==='UNREAD'?'isActive':''}><span>Unread</span><strong>{unreadTotal}</strong></button></div></header>{bookings.length===0?<div className="dashboardCard emptyState"><div>💬</div><h3>No conversations yet</h3><p>Booking messages and DriveDrop Support conversations will appear here.</p></div>:<div className="messagesLayout"><section className="dashboardCard conversationListPanel"><div className="conversationListHeading"><div><span className="dashboardEyebrow dark">Inbox</span><h2>Conversations</h2></div><span>{visibleBookings.length}</span></div><div className="conversationSearch"><span aria-hidden="true">⌕</span><input type="search" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="Search vehicle, person or reference" aria-label="Search conversations"/>{searchQuery&&<button type="button" onClick={()=>setSearchQuery('')} aria-label="Clear conversation search">Clear</button>}</div><div className="conversationList">{visibleBookings.length===0?<div className="emptyState conversationFilterEmpty"><div>{normalizedSearch?'⌕':'✓'}</div><h3>{normalizedSearch?'No conversations found':'No unread conversations'}</h3><p>{normalizedSearch?'Try a vehicle, person’s name or delivery reference.':'You are all caught up.'}</p></div>:visibleBookings.map(b=>{const support=b.status==='SUPPORT';const other=support?(me.role==='ADMIN'?b.customer.name:'DriveDrop Support'):me.role==='CUSTOMER'?b.transporter.name:b.customer.name;const latest=latestBookingMessage(b);const active=b.id===selectedId;const unread=unreadByBooking[b.id]||0;return <div key={b.id} className="conversationEntry"><button type="button" onClick={()=>selectBooking(b.id)} disabled={sending||deleting} aria-expanded={active} aria-controls={active?'selected-conversation':undefined} className={`conversationCard ${active?'isActive':''} ${unread?'hasUnread':''}`}><div className="conversationTop"><div><span className="conversationVehicle">{support?'DriveDrop Support':`${b.job.vehicleMake} ${b.job.vehicleModel}`}</span><small>{other}</small><small className="conversationCardReference">{support?'Private support conversation':bookingReference(b.id)}</small></div><div className="conversationMeta">{unread>0&&<span className="conversationUnread">{unread}</span>}<span>{support?'Support':label(b.status)}</span></div></div><p>{latest?(latest.body==='Photo'?'📷 Picture message':latest.body):'No messages yet'}</p>{latest&&<small className="conversationTime">{new Date(latest.createdAt).toLocaleString('en-GB',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'})}</small>}</button>{active&&<section id="selected-conversation" className="dashboardCard conversationPanel inlineConversationPanel">{selected?<><div className="messageStream">{messagesLoading?<p className="muted" role="status">Loading messages…</p>:messageLoadError&&messages.length===0?<p className="formNotice errorNotice" role="alert">{messageLoadError}</p>:messages.length===0?<div className="emptyState messageEmpty"><div>💬</div><h3>Start the conversation</h3><p>{selected.status==='SUPPORT'?'Send a private message to DriveDrop Support.':'Send a message or picture about this delivery.'}</p></div>:messages.map(m=>{const mine=m.senderId===me.id||m.sender.name===me.name;return <div key={m.id} className={`messageRow ${mine?'mine':'theirs'}`}><div className="messageBubble messageBubblePolished"><div className="messageSender">{mine?'You':m.sender.name}</div>{m.body&&m.body!=='Photo'&&<span className="messageBody">{m.body}</span>}{m.attachments&&m.attachments.length>0&&<div className={`messageAttachmentGrid count${Math.min(m.attachments.length,4)}`}>{m.attachments.map(a=><a key={a.id} href={`/api/messages/attachments/${a.id}`} target="_blank" rel="noreferrer" className="messageAttachment"><img src={`/api/messages/attachments/${a.id}`} alt="Message attachment"/></a>)}</div>}<small className="messageTimestamp">{new Date(m.createdAt).toLocaleString('en-GB')}</small></div></div>})}</div>{adminRecipientLabel&&<div className="formNotice" role="status"><strong>Message recipient · {adminRecipientLabel}</strong><div style={{marginTop:4,fontSize:11,fontWeight:600}}>This message remains in the booking conversation for dispute transparency.</div></div>}<form onSubmit={send} className="messageComposer"><div className="messageComposerTop"><div className="field messageTextField"><label>MESSAGE</label><textarea rows={3} maxLength={2000} disabled={sending} value={body} onChange={e=>setBody(e.target.value)} placeholder={selected.status==='SUPPORT'?'Write a private support message…':'Write a message about this booking…'}/></div>{selected.status!=='SUPPORT'&&<div className="messagePictureControl"><label className="messageAttachButton">📷 Add pictures<input ref={fileRef} disabled={sending} type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={e=>{const files=Array.from(e.target.files||[]).slice(0,4);setImages(files);if((e.target.files?.length||0)>4)setNotice('You can attach up to 4 pictures per message.')}}/></label><small>JPG, PNG or WebP · up to 4</small></div>}</div>{images.length>0&&<div className="selectedAttachments">{images.map((img,i)=><span key={`${img.name}-${i}`}>📷 {img.name.length>24?`${img.name.slice(0,21)}…`:img.name}</span>)}<button type="button" className="textAction" disabled={sending} onClick={()=>{setImages([]);if(fileRef.current)fileRef.current.value=''}}>Remove pictures</button></div>}{notice&&<div className="formNotice errorNotice">{notice}</div>}<div className="messageComposerActions">{selected.status!=='SUPPORT'&&<button type="button" className="textAction dangerAction" disabled={sending||deleting} onClick={()=>deleteConversation(selected.id)}>{deleting?'Deleting…':'Delete conversation'}</button>}<button className="btn orange" disabled={sending||deleting||(!body.trim()&&!images.length)}>{sending?'Sending…':images.length?`Send ${images.length>1?'message & pictures':'message & picture'}`:'Send message'}</button></div></form></>:<div className="emptyState messageSelectEmpty"><div>💬</div><h3>Select a conversation</h3><p>Choose a booking to read and reply to messages.</p></div>}</section>}</div>})}</div></section></div>}</main>;
}
