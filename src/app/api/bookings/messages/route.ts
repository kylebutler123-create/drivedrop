...
export async function POST(r:Request){
 ...
 const form=await r.formData();const bookingId=String(form.get('bookingId')||'');const requestedRecipientId=String(form.get('recipientId')||'').trim();const body=String(form.get('body')||'').trim();const files=form.getAll('images').filter((x):x is File=>x instanceof File&&x.size>0);
 ...
 const recipientId=u.role==='ADMIN'?(requestedRecipientId&&(requestedRecipientId===booking.customerId||requestedRecipientId===booking.transporterId)?requestedRecipientId:null):u.id===booking.customerId?booking.transporterId:u.id===booking.transporterId?booking.customerId:null;if(u.role==='ADMIN'&&requestedRecipientId&&!recipientId)return NextResponse.json({error:'Invalid message recipient'},{status:400});
 ...
 if(recipientId){const vehicle=`${booking.job.vehicleMake} ${booking.job.vehicleModel}`.trim();await createNotificationSafely({userId:recipientId,type:'MESSAGE',title:'New booking message',body:`${u.name} sent you ${uploads.length?'a message with a picture':'a message'} about the ${vehicle}.`,href:'/messages'})}
