...
export async function POST(r:Request){
 try{
  ...
  await createNotificationSafely({userId:result.customer.id,type:'QUOTE',title:'New transport quote received',body:`A verified transporter quoted £${(result.quote.pricePence/100).toFixed(2)} for your ${result.vehicleMake} ${result.vehicleModel}.`,href:'/customer'});
