...
export async function GET(){
 ...
 const transporters=await prisma.user.findMany({where:{role:'TRANSPORTER'},select:{id:true,name:true,email:true,phone:true,accountStatus:true,workRestricted:true,transporterVerification:{include:{documents:{orderBy:{createdAt:'desc'}}}}},orderBy:{createdAt:'desc'}});
 const rows=transporters.map(transporter=>transporter.transporterVerification?{...transporter.transporterVerification,transporter:{id:transporter.id,name:transporter.name,email:transporter.email,phone:transporter.phone,accountStatus:transporter.accountStatus,workRestricted:transporter.workRestricted}}:{id:`not-started:${transporter.id}`,status:'NOT_STARTED',businessName:'Not provided',companyNumber:null,phone:null,yearsOperating:null,website:null,businessAddress:null,submittedAt:null,reviewedAt:null,reviewNote:null,documents:[],transporter:{id:transporter.id,name:transporter.name,email:transporter.email,phone:transporter.phone,accountStatus:transporter.accountStatus,workRestricted:transporter.workRestricted}});
