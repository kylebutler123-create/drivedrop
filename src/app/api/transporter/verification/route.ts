import {NextResponse} from 'next/server';import {prisma} from '@/lib/prisma';import {currentUser} from '@/lib/auth';import {notifyAdminsSafely} from '@/lib/notifications';import {ensureTransporterInsuranceNotification,insuranceStatusForVerification} from '@/lib/insurance-expiry-notifications';import {z} from 'zod';
const S=z.object({businessName:z.string().min(2),companyNumber:z.string().max(30).optional(),businessAddress:z.string().min(5),phone:z.string().min(7).max(30),yearsOperating:z.number().int().min(0).max(150).optional(),website:z.string().url().optional().or(z.literal(''))});
export async function GET(){
 const u=await currentUser();
 if(!u||u.role!=='TRANSPORTER')return NextResponse.json({error:'Transporter access required'},{status:403});
 const verification=await prisma.transporterVerification.findUnique({where:{transporterId:u.id},include:{documents:{orderBy:{createdAt:'desc'}}}});
 if(!verification)return NextResponse.json(null);
 const insuranceStatus=insuranceStatusForVerification(verification);
 await ensureTransporterInsuranceNotification({transporterId:u.id,verificationId:verification.id,insuranceStatus});
