...
export async function PATCH(request:Request){
 ...
 const booking=await prisma.booking.findFirst({
  ...
  select:{id:true,payment:{select:{events:{where:{type:'PAYOUT_PAID'},orderBy:{createdAt:'desc'},take:1,select:{createdAt:true}}}}}
 });
 if(!booking)return NextResponse.json({error:'Paid delivery not found'},{status:404});
 const expectedEventKey=`PAID:${booking.payment?.events[0]?.createdAt?.toISOString()||''}`;
