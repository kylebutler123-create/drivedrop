...
function customerCompletedSummary(booking:any){
 const completedEvent=[...(booking.trackingEvents||[])].reverse().find((event:any)=>event.status==='DELIVERED');
 const payment=booking.payment;
 const evidenceCount=(booking.evidence||[]).length;
 return {title:`${booking.job?.vehicleMake||''} ${booking.job?.vehicleModel||''}`.trim(),transporter:booking.transporter?.name||'Transporter',registration:booking.job?.registration||'No registration',completedAt:completedEvent?.createdAt||booking.customerConfirmedAt,evidenceCount,paymentText:payment?.refundedPence>0?(payment.status==='REFUNDED'?'Refunded':'Partial refund'):payment?.status?label(payment.status):'Payment complete'};
}
