...
export default function AdminSectionSwitcher({userCount,transporterCount,disputeCount,userSection,verificationSection,disputeSection,operationsSection}:{userCount:number;transporterCount:number;disputeCount:number;userSection:ReactNode;verificationSection:ReactNode;disputeSection:ReactNode;operationsSection:ReactNode}){
 ...
 return <>
  <div className="dashboardSummary adminSectionSummary">
   ...
   <div role="button" tabIndex={0} aria-pressed={view==='OPERATIONS'} onClick={()=>toggle('OPERATIONS')} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();toggle('OPERATIONS')}}} style={cardStyle(view==='OPERATIONS')}><strong>🚗</strong><span style={labelStyle(view==='OPERATIONS')}>Bookings & deliveries</span></div>
  ...
  {view!=='ALL'&&<div className="dashboardFilterBar"><span>Showing {view==='USERS'?'user manager':view==='VERIFICATION'?'transporter verification':view==='DISPUTES'?'dispute management':'bookings & deliveries'} only</span><button className="textAction" onClick={()=>setView('ALL')}>Show everything</button></div>}
