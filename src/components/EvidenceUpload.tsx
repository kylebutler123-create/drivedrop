...
export default function EvidenceUpload({bookingId,type,onUploaded}:{bookingId:string,type:'COLLECTION'|'DELIVERY',onUploaded:()=>void|Promise<void>}){
 ...
 return <><input ref={input} type="file" accept="image/jpeg,image/png,image/webp" hidden onChange={e=>upload(e.target.files?.[0])}/><button className="btn orange" style={{marginRight:8}} disabled={busy} onClick={()=>input.current?.click()}>{busy?'Uploading…':`Add ${type==='COLLECTION'?'collection':'delivery'} photo`}</button></>
}
